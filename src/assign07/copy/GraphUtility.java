...
public class GraphUtility {
	...
	public static <Type> List<Type> shortestPath(List<Type> sources, List<Type> destinations, Type srcData,
		...
		Graph<Type> graph = graphMaker(sources, destinations);
		if(!graph.getVertices().containsValue(graph.getVertices().get(srcData))) {
			throw new IllegalArgumentException("Source data not found in graph.");
		}
		if(!graph.getVertices().containsValue(graph.getVertices().get(dstData))) {
			throw new IllegalArgumentException("Destination data not found in graph.");
		   }
		if(!graph.dfs(graph.getVertices().get(srcData), graph.getVertices().get(dstData))) {
